...
十大排序算法：插入排序，选择排序，冒泡排序，希尔排序，堆排序，快速排序，归并排序，计数排序，基数排序，桶排序。
...
|   shell   |   $n^2$    |    $1$     |   no   |
|   heap    | $n \log n$ |    $1$     |   no   |
|   quick   | $n \log n$ | $n \log n$ |   no   |
|   merge   | $n \log n$ |    $n$     |  yes   |
| counting  |  $n + k$   |  $n + k$   |  yes   |
|   radix   | $d(n + k)$ |  $n + k$   |  yes   |
|  bucket   |  $n + k$   |    $nk$    |  yes   |

相同的算法也有不同的实现，上述处理对象皆为数组，其中希尔排序的时间复杂度与序列有关，$k$ 分别为范围，基数，桶数，$d$ 为阶数。此外，对前六个就地排序给出了示例图。
